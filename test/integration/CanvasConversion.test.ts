...
describe('Integration Test: Canvas Conversion', () => {
	...
	it('should handle empty vault with no canvas files', async () => {
		...
		expect(fileDao.getFiles()).toHaveLength(0)
	})
	...
	it('should handle file deletion scenarios', async () => {
		...
		expect((fileDao.getFiles()).map((f) => f.path)).not.toContain(mdPath)
	})
	...
	it('should recreate .canvas.md file if it was manually deleted', async () => {
		...
		expect((fileDao.getFiles()).map((f) => f.path)).toContain(mdPath)
	})
	...
	it('should not modify already indexed files when re-transcribeing', async () => {
		...
		const mdPath = 'transcripts/Test.canvas.md'
		const initialModifiedTime = (fileAdapter.getFiles()).find(
			(f) => f.path === mdPath,
		)?.modifiedTime
		...
		const finalModifiedTime = (fileAdapter.getFiles()).find(
			(f) => f.path === mdPath,
		)?.modifiedTime
	...
	it('should handle canvas files in subfolders and multiple transcription runs', async () => {
		...
		expect(convertedContent).toContain('File: [Test.canvas](../sub%20folder/Test.canvas)')
		expect(convertedContent).toContain('![](../sub%20folder/Test.canvas)')
		...
		expect((fileDao.getFiles()).map((f) => f.path)).not.toContain(mdPath1)
	})
	...
	it.each([
		...
		async ({ postfix, folder }) => {
			...
			const expectedContent = readGeneratedTestFile('Test', '', folder)
