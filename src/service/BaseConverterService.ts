...
export abstract class BaseConverterService {
	...
	private findConvertedFileCandidates(allFiles: File[], sourceFiles: File[]): File[] {
		...
		const outputDirs = new Set(
			sourceFiles.map((source) => {
				...
				return outputPath.includes('/') ? outputPath.split('/').slice(0, -1).join('/') : ''
			}),
		)
	...
	protected async removeOrphanedFiles(
		convertedFiles: File[],
		sourceFiles: File[],
	): Promise<string[]> {
	...
	protected async createConvertedFiles(
		sourceFiles: File[],
		convertedFiles: File[],
	): Promise<{
		count: number
	...
	protected async modifyConvertedFiles(
		sourceFiles: File[],
		convertedFiles: File[],
	): Promise<{
		count: number
